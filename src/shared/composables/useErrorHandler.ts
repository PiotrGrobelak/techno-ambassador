...
export function useErrorHandler() {
  ...
  async function parseHttpError(response: Response): Promise<ClientError> {
    ...
    switch (statusCode) {
      case 400:
        type =
          (errorData.error as { code: string })?.code === 'VALIDATION_ERROR'
    ...
    return {
      ...
      message:
        ...
        'An error occurred',
      code: (errorData.error as { code: string })?.code,
      details: (errorData.error as { details: Record<string, unknown> })
        ?.details,
      statusCode,
    };
  }
   ...
   */
  async function processError(error: unknown): Promise<ClientError> {
    ...
    return {
      ...
      details: error as Record<string, unknown> | null,
    };
  }
  ...
  async function handleError(
    ...
    const clientError = await processError(error);
...
function isClientError(error: unknown): error is ClientError {
  return (
    ...
    'type' in error &&
    'message' in error &&
    Object.values(ClientErrorType).includes(
      (error as { type: ClientErrorType }).type
    )
  );
}
