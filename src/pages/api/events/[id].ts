...
export async function GET(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    try {
      const eventData: EventListItemDto =
  ...
  try {
    ...
    if (!user) {
      throw ApiErrors.unauthorized(
        'Authentication required. Please log in to update events.'
      );
    }
...
export async function DELETE(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    if (!user) {
      throw ApiErrors.unauthorized(
        'Authentication required. Please log in to delete events.'
      );
    }
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ message: 'Event successfully deleted' }),
        {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        }
